/**
 * PostgreSQL 连接池统一管理（db/postgres.ts）
 *
 ...
 *
 * 本目录对外统一出口是 db/index.ts（聚合 re-export postgres + redis），
 * 引用方从 db/index.js 导入即可，无需感知具体文件。
 */


/**
 * db/postgres.ts PostgreSQL连接池管理
 * 使用原生 node-postgres（pg）驱动：在此惰性创建全局唯一连接池（getPool），
 * repository层通过本模块访问数据库。
 * 生命周期：启动调用connectDatabase()探测建连；进程退出调用disconnectDatabase()释放池。
 * 对外统一由 db/index.ts 导出，业务无需感知内部文件。
 */

import { Pool } from 'pg';
import { logger } from '../utils/logger.js';
import { buildPostgresUrl } from './db.config.js';

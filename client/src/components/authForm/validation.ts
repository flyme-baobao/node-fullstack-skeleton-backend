 ...
 * 字段描述：
 *  - user_name 必填，字母/数字/下划线，6‑80位；
 *  - email / phone_number 选填：邮箱标准格式；手机号11位国内号码或0开头座机；
 *  - password 8‑72位ASCII可打印字符，需同时包含大小写字母、数字、特殊符号；
...
// 用户名：仅 字母、数字、下划线，6-80 位；不含空格、中文、全角、其他特殊可见符号
const USERNAME_REGEX = /^[A-Za-z0-9_]{6,80}$/;
...
// 密码：8-72 位 ASCII 可打印字符（不含空格、中文、全角），必须含 大小写字母 + 数字 + 特殊符号

...
| 401 | `40104` | `signin_credential_invalid` 凭证错误（带剩余次数倒计时） | signin 登录失败未达上限（限流窗口内，剩余次数取双维度较紧者） |
| 429 | `40105` | `signin_locked` 登录失败次数过多，临时锁定 | 同一账号×同一 IP 失败达 5 次，或同一账号跨设备合计达 10 次（15 分钟窗口） |
| 409 | `40901` | `account_exists` 账号已存在 | 注册冲突 |
| 403 | `40301` | `forbidden` | 预留，本版本不启用 |

> signin 的凭证失败（账号不存在 / 密码错误 / 账号禁用统一口径，防账号枚举）按限流进度分两版：
> 未达上限返回 40104（HTTP 401，文案带剩余次数倒计时，remaining = min(账号剩余, IP剩余)）；
> 任一维度计满即当场锁定，直接返回 40105（HTTP 429）。40103 保留给 token 校验等通用凭证失败路径。
